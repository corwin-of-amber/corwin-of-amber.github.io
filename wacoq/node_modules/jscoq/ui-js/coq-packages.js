...
class PackageManager {
    ...
    startPackageDownload(pkg_name) {
        ...
        if (bundle) {
            ...
            if (bundle.archive) {
                promise = this.loadPkg(pkg_name, bundle.archive.url.href);
                /*
                promise = 
    ...
    _bundleByURL(url) {
        var s = url.toString();
        for (let k in this.bundles) {
            var bundle = this.bundles[k];
            if (s == bundle.archive.url) return k;
        }
    }

    coqProgress(evt) {
        var url = new URL(evt.content.uri, new URL(this.coq._worker_script)),
            bname = this._bundleByURL(url);

        if (bname) {
            if (evt.done) {
                this.onBundleLoad(bname);
            }
            else {
                this.showPackageProgress(bname, evt.download);
            }
        }
    }
