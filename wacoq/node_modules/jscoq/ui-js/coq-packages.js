...
class PackageManager {
    ...
    initializePackageList(packages) {
        ...
        let mkpath = path => path == '-' ? PackageManager.defaultPkgPath() 
                             : path + (path.endsWith('/') ? '' : '/');
    ...
    populate() {
        ...
        return Promise.all(this.packages.map(async pkg => {
            var manifest = await (await fetch(`${pkg_path}/${pkg.name}.json`)).json();
            pkg.setArchive(`${pkg_path}/${pkg.name}.coq-pkg`);
        ...
        this.index.populate(pkg_path, this.packages.map(pkg => pkg.name));
    }
...
class PackageIndex {
    ...
    add(pkgInfo) {
        for (let mod of pkgInfo.modules || [])
